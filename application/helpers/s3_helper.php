...
final class S3Wrapper extends S3 {
	...
	public function stream_close() {
      ...
      $urlhere = basename($this->url['path']);
			$type = self::__getMIMEType($urlhere);
			if(!empty($this->context)) {
			$context = stream_context_get_options($this->context);
			if($context) {
				if(isset($context['s3']['acl'])) {
					$acl = $context['s3']['acl'];
				}
				if(isset($context['s3']['type'])) {
					$type = $context['s3']['type'];
				}
			}
			}
			self::putObject($this->buffer, $this->url['host'], $this->url['path'],$acl,array(),$type);
		}
		$this->position = 0;
		unset($this->buffer);
	}
...
$ci=& get_instance();
S3::$endpoint = $ci->config->item('s3_domain');
S3::setAuth($ci->config->item('s3_access_key'), $ci->config->item('s3_secret_key'));
